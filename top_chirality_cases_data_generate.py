...
penalt_list = [3.3]
v_list = [jnp.array([0.2,0.2,0.6]),jnp.array([0.15,0.15,0.7]),jnp.array([0.1,0.2,0.7]),jnp.array([0.15,0.2,0.65])]
type_initial_guess_list = [int(3)]
rmin_list = [4]
l_list = [0.74,0.8]# 0.7 is not enough, make it 0.74
itermax_ft_list = [int(4),int(6),int(8),int(12)]
...
path_to_save = '/s/dep/sms/w/FHG_Programmierbare_Materialien/PlainDutchWeave/chuc_2023_04/JAX_GPU/code/ProgMat_v1/collec_data/c4/chirality2/'

for count in range(len(parameters)):
        params = parameters[count]
        x = opi_micro_strucure(*params)
        file = 'rve_opt_' + str(count) + '.npy'
